...
|Versal HDIO |OBC の Versal PL Bank 302 に属する低速の I/O を示す。
|===
...
FTDI IC の Channel. A に接続可能であり、JP13~JP18 に挿入されているジャンパーソケットを『FTDI』シルク側へ切り替えることで利用が可能です。

==== Versal APU/RPU Console
Versal の APU/RPU のコンソールです。 +
FTDI IC の Channel. B および C へ、Versal の UART0 または UART1 にマッピングされたシリアル通信信号の接続が可能です。 +
接続可能なシリアル通信信号には Versal LPD MIO と Versal HDIO の 2種類があります。 +
各々の Channel に対して、JP19~JP24 に挿入されているジャンパーソケットを切り替えることにより、2種類の内 一方を選択して接続することが可能です。 +
各々のシリアル通信信号は独立しているため、一方のシリアル通信信号がテストで使用されることになっても、もう一方をコンソールとして利用することが可能です。
...
FTDI IC の Channel. D に接続可能であり、JP25~JP29 をに挿入されているジャンパーソケットを『FTDI』シルク側へ切り替えることで利用が可能です。

==== Versal SmartLynq+ HSDP
Versal の HSDP (High-Speed Debug Port) を使用し、AMD社製 SmartLynq+ モジュールを介して高速デバックおよびトレースが可能です。 +
JP34 へジャンパーソケットを挿入することで利用が可能です。
