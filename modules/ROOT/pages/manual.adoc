== はじめに
本マニュアルは、SC-OBC Module V1 を搭載できる I/F ボードの各種インタフェースに関する詳細を記述しております。 +
I/F ボードは、SC-OBC Module V1 へ電源を入れ、機能を動作させ、評価を円滑に行うことを目的に設計されています。 +
I/F ボードの仕様については、『SC-ESP-00107: テスト用IFボード仕様』を参照してください。
...
半二重で通信する場合、ジャンパーソケットを挿入してください。
|===
NOTE: 半二重で通信する場合に終端する際は、JP30 または JP31 のどちらかへジャンパーソケットを挿入してください。
...
CAN の終端抵抗が必要な場合、ジャンパーソケットを挿入してください。 +
...
対象の MIO は CON12 までの間にレベルシフタ (IC27) を介しているため、MIO の通信方向に従ってレベルシフタの通信方向を変更する必要があります。 +
...
デフォルトは JP57 へジャンパーソケットが挿入されています。
...
.3+|JP47~JP48 |Versal PMC MIO 0/1 DIR .3+|OFF
...
.3+|JP57~JP58 |FMC VADJ ( Versal VCCO 703 / IGLOO2 VDDI 8 ) .3+|1.2V
