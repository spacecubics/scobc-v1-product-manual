...
SC-OBC Module V1 は、キャリアボードから入力される 5 V を元に、モジュール内部で電源を生成し、電源ラインとして出力します。この出力は、キャリアボード上の回路の電源としての利用を想定しています。さらに、キャリアボード側の設計により、この出力電源をモジュールの VCCO 入力ピンへ接続し直すことで、モジュールの IO バンク用電源として利用することもできます。
...
上記表では、3 系統の IO バンク電源入力に対して 2 種類の Power Enable 信号を定義しており、任意の省電力制御を可能にしています。これらの信号はどちらも SC-OBC Module V1 から出力され、キャリアボード側へ配線されています。
...
==== キャリアボード上での PMC MIO 接続回路例

以下に、eMMC のように 1.8 V と 3.3 V 電源を必要とするデバイスを PMC MIO バンク 500 に接続する場合の、キャリアボード側の構成例を示します。
...
==== キャリアボード上での PL HDIO 接続回路例

以下に、3.3 V 電源を必要とするデバイスを PL HDIO バンク 302 に接続する場合の、キャリアボード側の構成例を示します。
